...
describe("ProjectController", () => {
  ...
  it("addTool assigns id and notifies", () => {
    ...
    expect(controller.getProjectData().tools.some((t) => t.id === "t1")).toBe(
      true,
    );
    expect(onChange).toHaveBeenCalled();
  });
  ...
  it("removeAgent removes normal agent and interactions/tools", () => {
    ...
    expect(controller.getProjectData().agents.some((a) => a.id === "a2")).toBe(
      false,
    );
  });
  ...
  it("removeTool removes tool and notifies", () => {
    ...
    expect(controller.getProjectData().tools.some((t) => t.id === "t2")).toBe(
      false,
    );
  });

  it("removeInteraction removes existing interaction", () => {
    const inNode = controller
      .getProjectData()
      .agents.find((a) => a.id === "user-input")!;
  ...
  it("assignToolToAgent updates tool.agentId and notifies", () => {
    ...
    expect(
      controller.getProjectData().tools.find((t) => t.id === "t3")!.agentId,
    ).toBe("user-input");
  });
