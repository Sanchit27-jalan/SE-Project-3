...
const ImportPreviewModal: React.FC<ImportPreviewModalProps> = ({
  ...
  const hasInputNode = ldlData.agents?.some(
    (agent) => agent.id === "user-input",
  );
  const hasOutputNode = ldlData.agents?.some(
    (agent) => agent.id === "user-output",
  );
      ...
      <DialogContent>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Box display="flex" gap={1}>
            ...
            />
            <Chip
              label={`${interactionCount} Connections`}
              size="small"
              variant="outlined"
            />
          </Box>
        ...
        <Box sx={{ width: "100%", mb: 2 }}>
                      ...
                      <Box display="flex" alignItems="center" gap={1} mb={1}>
                        ...
                        <Typography variant="subtitle1" fontWeight="bold">
                          ...
                          {(agent.id === "user-input" ||
                            ...
                            <Chip
                              label={
                                agent.id === "user-input" ? "Input" : "Output"
                              }
                              size="small"
                              color="primary"
                              sx={{ ml: 1 }}
                      ...
                      secondary={
                        <>
                          ...
                          <Typography variant="caption" color="text.secondary">
                            Protocol:{" "}
                            {interaction.protocol?.type || "Not specified"}
                          </Typography>
                        </>
                      }
                    />
          ...
          <TabPanel value={activeTab} index={3}>
            ...
            <Accordion>
              <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                <Typography>
                  Connections ({ldlData.interactions?.length || 0})
                </Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Paper sx={{ p: 2, bgcolor: "background.paper" }}>
